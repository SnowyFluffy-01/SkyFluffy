...
const data = {
 ...
 export default async function fetchData(r) {
  ...
  const skin = await axios.get('https://crafatar.com/skins/' + formatteduuid)
  data.account.name = ignRes.data.name;
  data.account.skin = skin


  const now = Date.now();
  const rankRes =  await axios.get(`https://api.hypixel.net/v2/player?uuid=${formatteduuid}`, {
    headers: {
        'API-Key' : process.env.NEXT_PUBLIC_API_KEY
    }
})
const ranked = !!rankRes?.data.player?.newPackageRank ?? rankRes.data.player.rank ?? rankRes.data.player.prefix
data.account.ranked = ranked


  const res = await axios.get(`https://api.hypixel.net/v2/skyblock/profiles?uuid=${formatteduuid}`, {
    headers: {
    'API-Key': process.env.NEXT_PUBLIC_API_KEY
    }
  })
...
const experience = res.data.profiles[ind].members[uuid].player_data.experience
...
// Inventory data 
let inv = res.data.profiles[ind].members[uuid].inventory?.inv_contents?.data 
let armor = res.data.profiles[ind].members[uuid].inventory?.inv_armor?.data
let ender = res.data.profiles[ind].members[uuid].inventory?.ender_chest_contents?.data
let equipment = res.data.profiles[ind].members[uuid].inventory?.equipment_contents?.data
let talisman = res.data.profiles[ind].members[uuid].inventory?.bag_contents?.talisman_bag?.data
