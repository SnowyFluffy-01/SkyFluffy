...
"§f": "#FFFFFF"
  ...
  const itemStats = {
    ...
    const regex = /(§.)?(damage|strength|crit.[chance|damage]+|ferocity|health|defense):.(..)?[+-]?(\d+[,.0-9]?\d*)(%)?/gim

export default async function parseInv(inv,armor,ender, talismans, equipment) {
  try {
        [inv,armor,ender, equipment, talismans].forEach((group, gIndex) => {
   const arrToUse = 
        gIndex === 0 ? items :     
        gIndex === 1 ? armorDisplay : 
        gIndex === 2 ? chest :        
        gIndex === 3 ? accBag :       
        equipmentDisplay;            
        ...
        if (group == equipment || group == armor || group == talismans ) {
         const matches = lore && lore.flatMap(line => {
